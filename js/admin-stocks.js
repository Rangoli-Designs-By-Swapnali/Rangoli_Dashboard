/* =========================================================
   ADMIN STOCKS MODULE
   Design-level stock, preparation queue, search, sort and filter
========================================================= */
...
function renderStocks(){
  ...
  /*
     Build a union of designs.json + Google Sheet stock records.
     This is important because stock must still appear even when
     designs.json is temporarily loading or a design name exists only
     in the Stock sheet.
  */
  const map=new Map();
  (Array.isArray(designs)?designs:[]).forEach(d=>{
    map.set(stockKey(d.name),{
      design:d.name,
      image:d.image||"",
  ...
  (Array.isArray(adminStocks)?adminStocks:[]).forEach(r=>{
    const key=stockKey(r.design);
    if(!key)return;
    const existing=map.get(key);
    if(existing){
      existing.stock=stockDisplayQuantity(r);
      existing.needToPrepare=stockNeedQuantity(r);
  ...
  let records=Array.from(map.values()).filter(x=>!q||x.design.toLowerCase().includes(q));

  const need=records.filter(x=>x.needToPrepare>0);
  const available=records.filter(x=>{
    if(filter==="available")return x.stock>0;
    if(filter==="zero")return x.stock<=0;
    if(filter==="need")return x.needToPrepare>0;
    return true;
  });

  need.sort((a,b)=>b.needToPrepare-a.needToPrepare||a.design.localeCompare(b.design));
  available.sort((a,b)=>{
    ...
    if(sort==="priceAsc")return stockLowestPrice(a.variants)-stockLowestPrice(b.variants)||a.design.localeCompare(b.design);
    if(sort==="priceDesc")return stockLowestPrice(b.variants)-stockLowestPrice(a.variants)||a.design.localeCompare(b.design);
...
function stockCardHTML(x){
  ...
  return `<div class="stock-card">
    ...
    <div class="stock-variants">${variants||"Sizes/prices available in catalogue"}</div>
