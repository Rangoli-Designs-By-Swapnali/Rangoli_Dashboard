...
function renderOrderTableHTML(list,all){
  ...
  return `<table class="order-table"><thead><tr><th>Order</th><th>Order Date</th><th>Customer</th><th>Items</th><th>Total</th><th>Payment</th><th>Status</th>${all?"<th>Actions</th>":""}</tr></thead><tbody>${list.map(o=>{
    ...
    return `<tr class="${orderVisualClass(o)}">
      ...
      <td>${adminMoney(o.total)}${orderRemaining(o)>0?`<br><span class="remaining-amount">Remaining: ${adminMoney(orderRemaining(o))}</span>`:""}</td>
      <td><span class="order-payment">${adminEsc(o.payment||"Pending")}</span>${orderRemaining(o)>0?`<br><span class="remaining-amount">Due: ${adminMoney(orderRemaining(o))}</span>`:""}</td>
...
function validateEditAdvancePayment(){
  ...
  if(payment!=="Advanced Received"&&payment!=="Partially Paid")return true;
  const t=editOrderItemsTotal(),a=Number(document.getElementById("editAdvanceAmount").value);
